---
title: "DAT205x: Introduction to Data Analysis using R. Lab1"
...
```
Let's view how our raw data looks like:  
```{r}
str(my_data)
...
my_data %>%
    ...
    labs(title = "Yearly Sales by Category", color = " Product\nCategory") +
    theme_classic() +
...
my_data %>%
    ...
    coord_flip() +
    labs(title = "Revenue by Country", y = "Total", x = "Country") +
    theme_classic() +
    theme(panel.background = element_rect(fill = "white", color = "white"),
          plot.title = element_text(face = "bold", hjust = 0.5),
          axis.text = element_text(size = 11, colour = "black"),
...
```{r}
saveRDS(my_data, "week1/data4week2.rds")
```