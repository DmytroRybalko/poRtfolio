---
title: "DAT205x: Introduction to Data Analysis using R. Lab8"
...
add_data <- read.table("./week8/Customer_demographics.txt", sep = ",", header = T) %>%
    as_data_frame()
```
Let's bring the customer database into your R Markdown file: add the **MaritalStatus** and **TotalChildren** to my_data
```{r eval = FALSE}
new_add <- add_data %>%
...
new_my <- my_data %>%
    ...
    left_join(new_add, c("Customer ID" = "Customer.ID"))
```

---

#### **Question 1:** For those customers who bought bikes, what are the top three (bought the most quantity) customer profiles (marital status and number of children)?
```{r ans_1}
ans_1 <- my_data %>%
    select(`Customer ID`, `Order Quantity`, `Product Category`) %>%
    left_join(add_data %>%
...
#### **Question 2:** For those customers who bought bikes, what are the top three (bought the most quantity) income brackets? (Rank from highest to Lowest) 
```{r ans_2}
ans_2 <- my_data %>%
    select(`Customer ID`, `Order Quantity`, `Product Category`) %>%
    left_join(add_data %>%
...
#### **Question 3:** For those customers who bought bikes, what are the top two (bought the most quantity) education levels?
```{r}
ans_3 <- my_data %>%
    select(`Customer ID`, `Order Quantity`, `Product Category`) %>%
    left_join(add_data %>%
...
#### **Question 4:** What is the percentage of the customers who bought Bikes and are house owners?
```{r}
ans_4 <- my_data %>%
    select(`Customer ID`, `Order Quantity`, `Product Category`) %>%
    left_join(add_data %>%
