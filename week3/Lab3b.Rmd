...
## Question 1: Which country’s trend is the most different when compared to
## the other countries?
```{r}
ans_1 <- my_data %>%
...
ans_2$`Product Category` <- factor(ans_2$`Product Category`,
                                     levels = c("Bikes", "Accessories", "Clothing"))

p <- ggplot(ans_2, aes(x = Year, y = Total, color = `Product Category`))
p + geom_line(size = 1.2) +
    geom_point(size = 2) +
    scale_y_continuous(labels = scales::dollar) +
    labs(x = "Year", y = "Total Revenue",
